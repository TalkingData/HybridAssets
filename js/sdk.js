window.talkingdata = {
      ...
      for (var index in talkingdata.configData) {
        ...
        if (sourcePath == elementPath) {
          ...
          if (e.target.tagName == "a") {
            console.log(talkingdata.configData[index].location, "meng");
            window.location.href = talkingdata.configData[index].location;
          }
  ...
  getAllAs: function() {
    let _as = document.querySelectorAll("a");
    _as.forEach(item => item.setAttribute("href", "javascript:void(0)"));
  },
  getConfig: function(
    webviewID,
    webviewX,
    ...
    for (var index in nodes) {
      ...
      if (tools.isVisible(node)) {
        ...
        var nodeInfo = {
          ...
          type: "h5_element",
          nodeName: node.nodeName, //add a
          webview_id: webviewID
        };
        //add forward Interception
        if (tools.elementForward(el)) {
          nodeInfo.location = tools.elementForward(el);
        }
